  ...
  const loadGmailAccounts = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        // Sync into profiles store so Campaign page sees them
        if (d.accounts?.length) {
          const nonGmail = profiles.filter(p => p.type !== 'gmail')
          const merged = [...nonGmail, ...d.accounts]
          // Only update if actually different
          const currentGmails = profiles.filter(p => p.type === 'gmail').map(p => p.user).sort().join(',')
          const newGmails = d.accounts.map(p => p.user).sort().join(',')
          if (currentGmails !== newGmails) {
            setProfiles(merged)
            pushToRedis()
          }
        }
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              ...
              <a href="/api/gmail?type=auth" className="btn-secondary text-xs px-3 py-1.5 flex items-center gap-1.5">+ Connect Gmail</a>
            </div>
          </div>
