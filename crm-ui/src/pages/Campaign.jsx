...
export default function Campaign() {
  ...
  async function getContent(lead, variantPool, index = 0) {
    ...
    if (variantPool && variantPool.length > 0) {
      ...
      let body = personalize(v.body)
      if (usePersonalization && lead.notes && lead.notes.trim()) {
        const hook = `Given that ${lead.company || 'your company'} ${lead.notes.trim().replace(/^(is |are |has |have )/i, '')}, `
        body = body.replace(/^(Hi [^\n,]+[,\n])(\n*)/i, (_, greeting, nl) => `${greeting}${nl}${hook}`)
      }
      return { subject: personalize(v.subject), body: body + buildAttachmentText() }
    }
    return {
